...
class Trainer():
    ...
    def train(self, input, real_val, idx=None, add=None):
        self.model.train()
        self.optimizer.zero_grad()
        output = self.model(input, idx=idx, add_input=add)
    ...
    def eval(self, input, real_val, add=None):
        self.model.eval()
        output = self.model(input, add_input=add)
        output = output.transpose(1,3)
        real = torch.unsqueeze(real_val,dim=1)
        predict = self.scaler.inverse_transform(output)
        loss = self.loss(predict, real, 0.0)
