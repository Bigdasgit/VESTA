...
parser.add_argument('--num_nodes',type=int,default=85,help='number of nodes/variables')
...
def main(runid):
    ...
    if args.use_additional_model:
        add_dataloader = load_dataset(args.add_data, args.batch_size, args.batch_size, args.batch_size)
    ...
    model = gtnet(args.use_additional_model, args.gcn_true, args.buildA_true, args.gcn_depth, args.num_nodes, args.add_num_nodes, 
                  ...
                  seq_length=args.seq_in_len, add_seq_length=args.add_seq_in_len, in_dim=args.in_dim, out_dim=args.seq_out_len,
    ...
    for i in range(1,args.epochs+1):
        ...
        if args.use_additional_model:
            add_dataloader['train_loader'].shuffle()
            add_data_iter = add_dataloader['train_loader'].get_iterator()
        for iter, (x, y) in enumerate(dataloader['train_loader'].get_iterator()):
            ...
            if args.use_additional_model:
                add_x, add_y = next(add_data_iter)
                add_trainx = torch.Tensor(add_x).to(device)
                add_trainx = add_trainx.transpose(1, 3)
            else:
                add_trainx = None
            ...
            for j in range(args.num_split):
                ...
                tx = trainx[:, :, id, :]
                ty = trainy[:, :, id, :]
                metrics = engine.train(tx, ty[:,0,:,:],id, add=add_trainx)
        ...
        s1 = time.time()
        if args.use_additional_model:
            add_val_data_iter = add_dataloader['val_loader'].get_iterator()
        for iter, (x, y) in enumerate(dataloader['val_loader'].get_iterator()):
            ...
            if args.use_additional_model:
                add_x, add_y = next(add_val_data_iter)
                add_testx = torch.Tensor(add_x).to(device)
                add_testx = add_testx.transpose(1, 3)
            else:
                add_testx = None
            metrics = engine.eval(testx, testy[:,0,:,:], add=add_testx)
    ...
    engine.model.load_state_dict(torch.load(args.save + "exp" + str(args.expid) + "_" + str(runid) +".pth"), strict=False)
    ...
    for iter, (x, y) in enumerate(dataloader['val_loader'].get_iterator()):
        ...
        if args.use_additional_model:
            ...
            add_testx = None
        with torch.no_grad():
            preds = engine.model(testx, add_input=add_testx)
    ...
    realy = realy.transpose(1, 3)[:, 0, :, :]
    if args.use_additional_model:
        add_test_data_iter = add_dataloader['test_loader'].get_iterator()
    for iter, (x, y) in enumerate(dataloader['test_loader'].get_iterator()):
        testx = torch.Tensor(x).to(device)
        testx = testx.transpose(1, 3)
        if args.use_additional_model:
            add_x, add_y = next(add_test_data_iter)
            add_testx = torch.Tensor(add_x).to(device)
            add_testx = add_testx.transpose(1, 3)
        else:
            add_testx = None
